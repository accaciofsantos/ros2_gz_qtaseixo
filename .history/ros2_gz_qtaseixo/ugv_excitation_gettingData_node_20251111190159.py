...
class IdentifyUGVNode(Node):
    ...
    def generate_excitation(self, counter):
        ...
        if self.tipo == 'prbs':
        ...
        elif self.tipo == 'validation_custom':
            ...
            if self.t < t_phase:
                ...
                # --- Fase 3: Giro alternado esquerda/direita ---
                val = 0.2 if int((self.t - 2*t_phase) / 3) % 2 == 0 else -0.2
                self.last_u_lin = 0.4        # velocidade linear menor
                self.last_u_ang = val        # giro alternado
