...
class UGVController(Node):
    ...
    def adrc_controller(self, dist, angle_err):
        ...
        r_ang = 6.0  # maior que r_lin, mas sem exagero
        beta1_ang = 3*r_ang
        beta2_ang = 3*r_ang**2
        beta3_ang = r_ang**3
        ...
        # ESO – angular (yaw)
        # --------------------------
        e_w = self.shortest_angle(angle_err, self.eso_ang_x1)  # CORREÇÃO: erro angular normalizado
        ...
        # Aplicar ESO sem limitar
        w_cmd_raw = u0_ang - self.eso_ang_x3

        # Saturação segura de acordo com capacidade do robô
        max_w = 2.0
        w_cmd = np.clip(w_cmd_raw, -max_w, max_w)
