...
jitter_x = 0.20                 # deslocamento aleatório para coluna 2
jitter_y = 0.25
extra_trees_col3 = 3            # número de árvores adicionais para alongar a coluna 3
extra_spacing_col3 = 0.8        # espaçamento entre árvores extras na coluna 3
...
for row in range(num_rows):
    ...
    # pivô para rotação da 4ª fileira
    pivot_x = 0
    pivot_y = y_base
    ...
    for col in range(num_cols):
        ...
        # === Coluna 3: alongar a coluna com árvores extras ao longo do Y ===
        elif col == 2:
            # árvore principal
            x2, y2, yaw = x, y_base, 0.0

        # === Coluna 4: rotaciona toda a fileira ===
        elif col == 3:
            x2 = pivot_x + (x - pivot_x) * math.cos(yaw_row4) - (y_base - pivot_y) * math.sin(yaw_row4)
            y2 = pivot_y + (x - pivot_x) * math.sin(yaw_row4) + (y_base - pivot_y) * math.cos(yaw_row4)
            yaw = 0.0
    ...
    <include>
      ...
      <pose>{x2:.3f} {y2:.3f} 0 0 0 {yaw:.3f}</pose>
        ...
        # === Árvores extras para alongar a coluna 3 ===
        if col == 2:
            for extra in range(extra_trees_col3):
                y_extra = y_base + (extra + 1) * extra_spacing_col3
                tree_includes += f"""    <!-- Tree {tree_id}: row {row+1}, col {col+1} extra -->
    <include>
      <name>tree_{tree_id}</name>
      <uri>{tree_model_uri}</uri>
      <pose>{x2:.3f} {y_extra:.3f} 0 0 0 {yaw:.3f}</pose>
    </include>\n"""
                tree_id += 1
