...
yaw_col1 = math.radians(5.0)   # 5 graus de inclinação global
jitter_x = 0.20                # jitter ±0.20m
jitter_y = 0.25                # jitter ±0.25m
...
for row in range(num_rows):
    ...
    for col in range(num_cols):
        ...
        # === Coluna 1: inclinação angular ===
        elif col == 0:
            x2 = x*math.cos(yaw_col1) - y*math.sin(yaw_col1)
            y2 = x*math.sin(yaw_col1) + y*math.cos(yaw_col1)
            yaw = yaw_col1

        # === Colunas 2 e 3: jitter ===
        elif col in [1, 2]:
...
print(f"[OK] scenario3 gerado em {world_path}")