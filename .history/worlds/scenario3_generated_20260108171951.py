...
jitter_x = 0.20                 # deslocamento aleatório na coluna 2
jitter_y = 0.25
extra_spacing_x_col3 = 0.8      # espaçamento adicional para alongar a coluna 3 em X
...
for row in range(num_rows):
    ...
    for col in range(num_cols):
        # Ajuste de X para a coluna 3 (alongamento horizontal)
        if col == 2:
            x = col * dx + extra_spacing_x_col3
        else:
            x = col * dx
        ...
        # Coluna 4 → rotaciona toda a fileira
            ...
            x_orig = x
            y_orig = y_base
            x2 = pivot_x + (x_orig - pivot_x) * math.cos(yaw_row4) - (y_orig - pivot_y) * math.sin(yaw_row4)
            y2 = pivot_y + (x_orig - pivot_x) * math.sin(yaw_row4) + (y_orig - pivot_y) * math.cos(yaw_row4)
            yaw = 0.0
...
# === HEADER do mundo (igual scenario2) ===
